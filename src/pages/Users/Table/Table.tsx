...
const CustomTable = () => {
  const { users, isLoading, pagination } = useListUsers();
  ...
  return (
    <div className='p-2 bg-white rounded-lg shadow-lg'>
      ...
      <Table>
        ...
        {!isLoading && users && users.length === 0 && <TableEmpty colSpan={6} />}
          ...
          <TableBody>
            {users?.map((user) => (
              <TableRow key={user.id}>
                ...
                <TableCell>{customFormatDate(new Date(user.createdAt))}</TableCell>
                <TableCell>
